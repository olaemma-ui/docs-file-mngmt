      ...
      <main className="flex-1 md:ml-64 transition-all duration-300">
        ...
        <header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="px-4 md:px-8 py-4 md:py-6">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl md:text-3xl font-mono font-semibold text-foreground">Dashboard</h1>
        ...
        <div className="p-4 md:p-8 space-y-8">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Upload Document", icon: <FileText className="w-5 h-5" />, href: "/documents" },
              { label: "Create Folder", icon: <FileText className="w-5 h-5" />, href: "/folders" },
              { label: "Invite Users", icon: <Users className="w-5 h-5" />, href: "/users" },
              { label: "View Analytics", icon: <TrendingUp className="w-5 h-5" />, href: "/analytics" },
            ].map((action, index) => (
              <Card
                key={index}
